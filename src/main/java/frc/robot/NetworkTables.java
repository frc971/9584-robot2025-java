...
public class NetworkTables {
    ...
    private static final List<ConstantEntry> constantEntries = new ArrayList<>() {{
        ...
        add(new ConstantEntry() {{
            ...
            defaultValue.doubleValue = 1.2 * 2 * Math.PI;
        }});
        add(new ConstantEntry() {{
        ...
        add(new ConstantEntry() {{
            ...
            defaultValue.doubleValue = 30.0 * 1.2;
        }});
        add(new ConstantEntry() {{
    ...
    public Measure<edu.wpi.first.units.AngularVelocity> getAngularRateValue(ConstantId id) {
        ConstantEntry entry = constantEntries.get(id.ordinal());
    ...
    public Measure<edu.wpi.first.units.Velocity<edu.wpi.first.units.Distance>> getVelocityValue(ConstantId id) {
        ConstantEntry entry = constantEntries.get(id.ordinal());
    ...
    public Measure<edu.wpi.first.units.Acceleration<edu.wpi.first.units.Distance>> getAccelerationValue(ConstantId id) {
        ConstantEntry entry = constantEntries.get(id.ordinal());
        if (entry.type != ConstantType.Acceleration) {
            throw new RuntimeException("Constant type mismatch for " + entry.networkTableKey);
        }
        return MetersPerSecondPerSecond.of(table.getEntry(entry.networkTableKey).getDouble(entry.defaultValue.doubleValue));
    }

    public Measure<edu.wpi.first.units.AngularAcceleration> getAngularAccelerationValue(ConstantId id) {
        ConstantEntry entry = constantEntries.get(id.ordinal());
        if (entry.type != ConstantType.AngularAcceleration) {
            throw new RuntimeException("Constant type mismatch for " + entry.networkTableKey);
        }
        return RadiansPerSecondPerSecond.of(table.getEntry(entry.networkTableKey).getDouble(entry.defaultValue.doubleValue));
    }

    public Measure<edu.wpi.first.units.Time> getTimeValue(ConstantId id) {
        ConstantEntry entry = constantEntries.get(id.ordinal());
    ...
    public Measure<edu.wpi.first.units.Current> getCurrentValue(ConstantId id) {
        ConstantEntry entry = constantEntries.get(id.ordinal());
